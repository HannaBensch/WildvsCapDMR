...
ggarrange(
  ...
  asvs %>% 
  inner_join(taxonomy, by = 'asv') %>%
  left_join(top10class %>% transmute(class, topph = class), by = 'class') %>%
  group_by(topph, sample) %>%
...
```{r relabsbox2, fig.cap = '**Figure 2.** Relative abundances of most phylum with 100 % prevalence. Samples are either "Captive" (orange) or "Wild" (green) by color and with boxplot show spread of raw data.', fig.width=12, fig.height = 6, message=FALSE, warning=FALSE, echo = FALSE}
plot2 <-asvs %>%
  inner_join(taxonomy, by = 'asv') %>%
  left_join(top7phyla %>% transmute(phylum, topph = phylum), by = 'phylum') %>%
  replace_na(list('topph' = 'Other phyla')) %>%
...
asvs %>%
  inner_join(taxonomy, by = 'asv') %>%
  left_join(top10fam %>% transmute(family, toppf = family), by = 'family') %>%
  filter(!is.na(toppf)) %>%
